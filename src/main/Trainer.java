...
abstract class Trainer extends TurnablePiece {
	...
	Trainer(String name, int xStart, int yStart) {
		super(xStart, yStart);
		this.name = name;
	...
	boolean pickPokemonOnDeck(boolean mustSwap) {
	...
	Pokemon getPokemonOnDeck() {
		return pokemonOnDeck;
	}

	void setPokemonOnDeck(Pokemon pokemonOnDeck) {
	...
	void takeOutFirstPokemon() { // first pokemon in party is sent out
		currentPokemon = party.getPokemon(1);
		pokemonOnDeck = null;
	}

	void putAwayAllPokemon() {
		currentPokemon = null;
		pokemonOnDeck = null;
	}
