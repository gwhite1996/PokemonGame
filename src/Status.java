...
public enum Status{
	...
	private static boolean putToSleep(Pokemon pokemon){
		...
		pokemon.statusTurnsRemaining = new Random().nextInt(7) + 1; // random in 1 to 7
		return true;
	}
	...
	public static boolean removeStatus(Pokemon pokemon){
		...
		case FROZEN:
			...
			break;
		case PARALYZED:
			removeParalysis(pokemon);
			break;
		case POISONED:
			removePoison(pokemon);
			break;
		case NONE:
			return false;
		default:
			System.out.println("Invalid status in removeStatus()");
			return false;
		}
